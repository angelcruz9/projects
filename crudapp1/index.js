...
class HouseService{
    static url = "https://635d2290cb6cf98e56adaf5a.mockapi.io/houseapp"
        ...
        for(let house of houses){
           ...
           for (let room of house.rooms){
            $(`#${house.id}`).find('.card-body').append(
                `<p>
                <span id="name-${house.id}"><strong> Name: </strong> ${room.name}</span>
                <span id="area-${house.id}"><strong> Area: </strong> ${room.area}</span>
                <button class="btn btn-danger" onclick="DOMManager.deleteRoom('${house.id}', '${house.id}')">Delete Room</button>
                `
            )
           }
        }
